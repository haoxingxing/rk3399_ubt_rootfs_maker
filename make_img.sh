...
apt-get install -y  language-pack-en-base screen ntp sudo ssh net-tools ethtool pkg-config wireless-tools ifupdown network-manager iputils-ping bash-completion htop synaptic alsa-utils nano vim git udev build-essential sshfs openssh-server telnetd bluez telnet nmon curl language-pack-zh
...
sudo ./tools/qemu-x86_64 -L ./tools/make_ext4fs -s -l3072M -a root -L rootfs rootfs.img ubuntu-mount
